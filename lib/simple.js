...
const { convertSticker, mp4ToWebp } = require('./function')
 ...
 */
    ...
    */
    ...
    async send2ButtonVideo(jid, content, buffer, footer, button1, row1, options = {}) {
      const m = await this.prepareMessage(jid, buffer, MessageType.video)
      ...
      try {
        await this.sendMessage(jid, buttonMessage, MessageType.buttonsMessage, { ...options })
      } catch(e) {
        throw e
      }
    }
    ...
    // video
    async send2ButtonVideo(jid, buffer, content, footer, button1, row1, button2, row2, quoted, options = {}) {
      return await this.sendMessage(jid, {
        contentText: content,
        footerText: footer,
        buttons: [
     ...
     * @param {String} pack
     */
    async sendSticker(jid, path, author, pack, quoted) {
      let file = await this.getFile(path)
      let mime = file.mime
      if (mime === 'image/jpeg') { 
      let base64 = `data:image/jpeg;base64,${file.data.toString('base64')}`
      let stc = await convertSticker(base64, author, pack);
      let stcBuffer = new Buffer.from(stc, 'base64');
      this.sendMessage(jid, stcBuffer, MessageType.sticker, {quoted: quoted})
      } else if (mime === 'video/mp4') {
      let base64 = `data:video/mp4;base64,${file.data.toString('base64')}`
      let stc = await mp4ToWebp(base64, author, pack)
      this.sendFile(jid, stc, '', '', false, quoted, { asSticker: true })
      }
    }
