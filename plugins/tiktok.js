cmd.get("tiktok", async(m, args) => {
	...
	cmd.send2ButtonVideo(m.chat,  await (await fetch(tiktok.nowm)).buffer(), `${tiktok.caption}`, 'By Tiktok Downloader', 'With Watermark', `>wm ${tiktok.wm}`, 'Only Audio', `>ttaudio ${tiktok.music}`, m)
  	} catch(err) {
  		console.log(err)
